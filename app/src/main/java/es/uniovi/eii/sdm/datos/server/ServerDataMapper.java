...
public class ServerDataMapper {
    ...
    public static List<Pelicula> convertMovieListToDomain(List<MovieData> movieData) {
        ...
        for (MovieData peliApi : movieData) {
            ...
            Pelicula p = new Pelicula(peliApi.getId(), peliApi.getTitle(), peliApi.getOverview(), new Categoria("", ""), "", peliApi.getReleaseDate(), urlCaratula, urlFondo, "");
            lpeliculas.add(p);
            Log.d("Peli: ", p.toString());
        }
